...
export const IsUserFullySignedIn = (context) => {
    return (
        ...
        typeof context.user.username != 'undefined' &&
        typeof context.user.user_id != 'undefined'
    );
};
...
export async function SetUser(context, req) {
    return new Promise(async (resolve, reject) => {
        if (context.user && req.body && req.body.username) {
            ...
            if (Constants.contentModeratorEnabled) {
                if (
                    !process.env.CONTENT_MOD_ENDPOINT ||
                    !process.env.CONTENT_MOD_KEY
                ) {
                    context.send({
            ...
            const user_result = await azMySQLManager.createNewUser(
                context.user.email,
                username
            );
            if (user_result) {
